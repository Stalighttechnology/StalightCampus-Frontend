...
export const getDistrictsForState = (state: string) => {
  // Try direct match
  if (INDIAN_STATES_AND_DISTRICTS[state]) {
    return INDIAN_STATES_AND_DISTRICTS[state];
  }
  // Try case-insensitive and trimmed match
  const normalizedState = state.trim().toLowerCase();
  const foundState = Object.keys(INDIAN_STATES_AND_DISTRICTS).find(s => s.toLowerCase() === normalizedState);
  if (foundState) {
    return INDIAN_STATES_AND_DISTRICTS[foundState];
  }
  // If state is not found or has some extra string like "(29)", we can do partial matching
  const partialMatch = Object.keys(INDIAN_STATES_AND_DISTRICTS).find(s => normalizedState.includes(s.toLowerCase()));
  if (partialMatch) {
    return INDIAN_STATES_AND_DISTRICTS[partialMatch];
  }
  return [];
};
