...
import { applyAdminLeave, getAdminApplyLeaveBootstrap } from '../../../utils/transport_api';
...
const LeaveRequests = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  ...
  const pagination = usePagination({
    queryKey: ['transport-adminLeaves'],
    pageSize: 10
  });
  ...
  useEffect(() => {
    ...
    getAdminApplyLeaveBootstrap(qs).
      then((res) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const res = await applyAdminLeave(requestData);
          ...
          <CardHeader className="p-4 sm:p-6 border-b bg-muted/30 flex flex-row items-center justify-between gap-4">
            ...
            <div className="flex-shrink-0 mt-2 sm:mt-0">
              <Popover open={filterOpen} onOpenChange={setFilterOpen}>
                ...
                <PopoverContent className={`w-40 sm:w-48 p-2 sm:p-3 lg:p-4 ${theme === 'dark' ? 'bg-card text-foreground border-border' : 'bg-white text-gray-900 border-gray-200'}`}>
