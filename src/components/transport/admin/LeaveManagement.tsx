...
import { manageDriverLeaves, processDriverLeave } from "../../../utils/transport_api";
...
const DriverLeavesManagement = () => {
  ...
  const fetchLeaves = async (month?: string, page: number = 1) => {
    ...
    try {
      ...
      const response = await manageDriverLeaves(qs);
      ...
      if (dataSource && dataSource.success) {
        ...
        dataSource.leaves.map((leave: any) => ({
          ...
          name: leave.applicant_name || leave.driver_name || leave.hod?.username || "N/A",
          role: leave.role || "driver",
  ...
  return (
    <>
      ...
      <div className={`w-full min-h-full ${theme === 'dark' ? 'bg-background text-foreground' : 'bg-gray-50 text-gray-900'}`}>
        <Card id="transport-leaves-card" className={theme === 'dark' ? 'bg-card border border-border flex flex-col w-full shadow-sm' : 'bg-white border border-gray-200 flex flex-col w-full shadow-sm'}>
          <CardHeader id="transport-leaves-header-section" className="leave-card-header pb-3 md:pb-4 border-b border-inherit">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
              <div>
                ...
                <p className={`leave-card-desc text-sm ${theme === 'dark' ? 'text-muted-foreground' : 'text-gray-500'}`}>
                  Review and approve leave requests from Transport Staff & Drivers
                </p>
              </div>
...
export default DriverLeavesManagement;