...
const LoginMobile = ({ setRole, setPage, setUser }: LoginMobileProps) => {
  ...
  return (
    <div className="min-h-[100dvh] pt-[env(safe-area-inset-top,0px)] pb-[calc(env(safe-area-inset-bottom,0px)+8px)] overflow-x-hidden overflow-y-auto flex flex-col w-full bg-transparent relative">
      {/* HEADER */}
      <div className="text-center space-y-0.5 pt-4 pb-2 min-h-[700px]:pt-6 min-h-[700px]:pb-3 shrink-0 px-4">
      ...
      {/* MAIN CONTENT */}
      <div className="flex-1 flex flex-col justify-between px-4 max-w-sm w-full mx-auto py-1 min-h-[700px]:py-3">
        <div className="flex flex-col gap-2 min-h-[700px]:gap-4">
          {/* ILLUSTRATION CARD */}
          <div className="w-full bg-[#EDE9FE] border border-white/40 rounded-2xl flex items-center justify-center h-[16vh] min-h-[90px] max-h-[140px] mb-1 overflow-hidden">
            <img
              src="/image.png"
              alt="classroom"
          ...
          <div className="w-full bg-[#F9FAFB] rounded-2xl shadow-md p-4 min-h-[700px]:p-5 transition-all duration-300 hover:shadow-lg">
            ...
            <div className="mb-2 min-h-[700px]:mb-3">
              ...
              <p className="text-center text-gray-400 text-xs mt-0.5 hidden min-h-[650px]:block">Sign in to access your account</p>
            </div>

            {error && (
              <div className="bg-red-50 text-red-500 text-xs text-center py-1.5 px-3 rounded-lg mb-2 border border-red-100">
                {error}
              </div>
            )}
            ...
            <div className="space-y-2.5 min-h-[700px]:space-y-3">
              ...
              <div>
                ...
                <div className={`flex items-center gap-3 bg-slate-50 border ${error ? 'border-red-300' : 'border-slate-200'} rounded-xl h-11 min-h-[700px]:h-12 px-4`}>
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="flex-1 border-none bg-transparent focus:outline-none focus:ring-0 text-slate-700 placeholder:text-slate-400 text-sm"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className={`flex items-center gap-3 bg-slate-50 border ${error ? 'border-red-300' : 'border-slate-200'} rounded-xl h-11 min-h-[700px]:h-12 px-4 relative`}>
              ...
              <div className="text-right mt-0.5">
                <button
                  ...
                  disabled={loading}
                  className="text-[11px] text-purple-600 hover:text-purple-700 hover:underline disabled:opacity-50 cursor-pointer transition-colors duration-200"
                >
                  Forgot Password?
                </button>
              </div>
              ...
              <button
                onClick={handleLogin}
                disabled={loading}
                className="w-full h-10 rounded-xl bg-gradient-to-r from-[#6D28D9] to-[#7C3AED] text-white font-semibold text-sm shadow-lg shadow-purple-500/20 mt-1 hover:scale-[1.02] hover:shadow-lg active:scale-95 transition-all duration-200 disabled:opacity-70 cursor-pointer flex items-center justify-center"
              >
                {loading ? (
        ...
        {/* FOOTER */}
        <div className="text-center text-white text-[10px] pb-2 mt-1">
