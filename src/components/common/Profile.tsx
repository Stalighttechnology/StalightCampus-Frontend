...
const Profile = ({ role, user }: ProfileProps) => {
  ...
  return (
    <div id={role === 'transport_admin' ? 'transport-profile-header' : role === 'library_admin' ? 'library-profile-header' : role === 'driver' ? 'driver-profile-header' : undefined} className="min-h-screen flex justify-center items-start">
      <Card className={`w-full max-w-none mx-auto my-2 ${theme === 'dark' ? 'bg-card text-foreground' : 'bg-white text-gray-900'}`}>
        <CardHeader id={role === 'transport_admin' ? 'transport-profile-action-header' : role === 'library_admin' ? 'library-profile-action-header' : role === 'driver' ? 'driver-profile-action-header' : undefined} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 border-b">
