...
import { markDeveloperMyAttendance, getDeveloperMyAttendanceRecords, MarkDeveloperMyAttendanceRequest, DeveloperMyAttendanceRecord } from "@/utils/developer_api";
...
const DeveloperMyAttendance = () => {
  ...
  const [todayRecord, setTodayRecord] = useState<DeveloperMyAttendanceRecord | null>(null);
  const [recentRecords, setRecentRecords] = useState<DeveloperMyAttendanceRecord[]>([]);
  ...
  const [historyRecords, setHistoryRecords] = useState<DeveloperMyAttendanceRecord[]>([]);
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      const response = await getDeveloperMyAttendanceRecords({ page: 1, page_size: 7, start_date: startDate });
  ...
  const fetchHistoryPage = async (page: number) => {
    try {
      ...
      const response = await getDeveloperMyAttendanceRecords(params);
      if (response.success && response.data) {
  ...
  const handleToggleAttendance = async (status: "present" | "absent", action?: "check_in" | "check_out") => {
    ...
    try {
      ...
      const requestData: MarkDeveloperMyAttendanceRequest & any = {
        status,
        action,
      ...
      const response = await markDeveloperMyAttendance(requestData);
      ...
      if (response.success) {
        ...
        if (selfDeclarationResult.isConfirmed && selfDeclarationResult.value) {
          ...
          const retryResponse = await markDeveloperMyAttendance({
            ...requestData,
  ...
  const handleOffCampusDuty = async () => {
    ...
    try {
      ...
      setLoadingMessage('Syncing with server...');
      const response = await markDeveloperMyAttendance({
        status: 'present',
        action: 'check_in',
        off_campus_reason,
        latitude,
  ...
  return (
    <div className={` md: space-y-6 ${theme === 'dark' ? 'bg-background text-foreground' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card id="developer-attendance-history" className={`flex flex-col h-full ${theme === 'dark' ? 'bg-card text-foreground' : 'bg-white text-gray-900'}`}>
          <CardHeader id="developer-attendance-history-header" className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 pb-4 min-h-[72px] sm:h-[80px] gap-3">
...
export default DeveloperMyAttendance;