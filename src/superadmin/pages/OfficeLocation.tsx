...
import { manageOfficeLocation } from '@/utils/admin_api';
...
const CampusLocationManager: React.FC = () => {
  ...
  const loadLocations = async () => {
    setLoading(true);
    try {
      const response = await manageOfficeLocation();
      ...
      if (response.success || response.locations || response.results) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await manageOfficeLocation(data, editingLocation.id, 'PUT') :
      await manageOfficeLocation(data, undefined, 'POST');
